...
# recoded <- recode_predictors(insurance_train)

#write.csv(recoded, 'insurance_training_data_T.csv', row.names = F)
...
ins <- read.csv("https://raw.githubusercontent.com/621-Group2/HW4/master/insurance_training_Impute.csv")
...
m1<- calc_metrics("Model1", model1, test_all, train_all, show=F)
...
m2<- calc_metrics("Model2 - All Pred, Some Transformed", model2, test_allT, train_allT, show=F)
...
mod3 <- glm(target ~ .,  family=binomial(), data=train_all)
...
m3<- calc_metrics("Model3 - Step", model3, test_all, train_all, show=F)
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
model4 <- glmrob(target ~ KIDSDRIV + MALE + MARRIED + SINGLE_PARENT + 
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
model3fit <- predict(model3,newdata=test_all[,2:ncol(test_all)],type='response')
...
library(ROCR)
p <- predict(model3, newdata=test_all[,2:ncol(test_all)], type="response")
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
model4fit <- predict(model4,newdata=test_all[,2:ncol(test_all)],type='response')
...
p <- predict(model4, newdata=test_all[,2:ncol(test_all)], type="response")
pr <- prediction(p, test_all$target)
prf <- performance(pr, measure = "tpr", x.measure = "fpr")
...
```{r load_dev_train, include=FALSE}
...
insurance_train <- read.csv( "https://raw.githubusercontent.com/621-Group2/HW4/master/insurance_training_Impute.csv")  


# remove target amounts that are not postive
insurance_train <- filter(insurance_train, TARGET_FLAG == 1)

#Remove negative observation
insurance_train <- insurance_train[insurance_train$CAR_AGE >= 0, ]

# drop the target_flag response variable
insurance_train$TARGET_FLAG <- NULL


# create the dev_train and dev_test datasets using the non-transformed variables
idx <-createDataPartition(y=insurance_train$TARGET_AMT,p=0.7,list=FALSE)
dev_train <-insurance_train[idx,]
dev_test <-insurance_train[-idx,]
...
model1 = lm(TARGET_AMT ~ ., data = dev_train)
summary(model1)


#Diagnostic plots
autoplot(model1, which = 1:6, colour = 'dodgerblue3',
         smooth.colour = 'red', smooth.linetype = 'dashed',
         ad.colour = 'black',
         label.size = 3, label.n = 5, label.colour = 'blue',
         ncol = 3)

model1_metrics <- calc_metrics_lm("lm-Model1", model1, dev_test, dev_train)
...
AIC (Akaike Information Criterion) for Model 2 = `r AIC(model2)`  
BIC (Bayesian Information Criterion) for Model 2 = `r BIC(model2)`  
...
Vars <- names(unlist(model2[[1]]))                           
Vars<- Vars[!Vars %in% "(Intercept)"]  # remove intercept 

x <- setNames(data.frame(coef(model2)), 'Coefficient')
x$Variable <- rownames(x)
...
model2_metrics <- calc_metrics_lm("lm-Model2", model2, dev_test, dev_train)
...
insurance_train_test <- insurance_train_test[,-c(8:14, 27:31, 37:35)]
